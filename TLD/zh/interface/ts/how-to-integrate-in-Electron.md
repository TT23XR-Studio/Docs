# TaiLerDownloader TypeScript 库 Electron 集成指南

本文档指导您将 **TaiLerDownloader** TypeScript 库（基于 [***koffi***](https://koffi.dev) 调用 TT 高速下载器动态库）集成到 Electron 应用中。该库可通过 npm 包 ***TaiLerDownloader*** 安装，并用于在 Electron 主进程中创建、控制多任务下载，通过回调接收进度事件。

> [!TIP]
> 本文档介绍的是 **TLD**（Rust 版本）的集成方式。
> 
> API 接口与 [TTHSD Golang](https://github.com/sxxyrry/TTHighSpeedDownloader) 版本完全兼容，只需替换动态库即可迁移。
> 
> TTHSD Golang 已停止开发，建议使用 TLD 以获得更好的性能。

---

## 1. 概述

***TaiLerDownloader*** 库通过 Node.js 的原生模块 ***koffi*** 直接调用 C 动态库（***TaiLerDownloader.dll***/***TaiLerDownloader.dylib***/***TaiLerDownloader.so***），实现高性能下载功能。在 Electron 中，建议将该库运行于 **主进程**，因为它依赖于原生模块，且需要与文件系统交互。通过 IPC（进程间通信），您可以安全地将下载进度和控制能力暴露给渲染进程。
...
- TT 高速下载器动态库文件（**注意文件名均为小写**）：
  - Windows: ***TaiLerDownloader.dll***
  - macOS: ***TaiLerDownloader.dylib***
  - Linux: ***TaiLerDownloader.so***
- 动态库架构（32/64 位）必须与 Electron 应用的架构一致

> [!TIP]
> 您可以在 [TLD GitHub Releases](https://github.com/TaiLerDownloader/TaiLerDownloader/releases/latest) 页面下载最新版本的动态库文件。
...
```bash
npm install TaiLerDownloader
npm install --save-dev electron-rebuild
```

***TaiLerDownloader*** 已内置所需的 ***koffi*** 依赖，并提供了完整的 TypeScript 类型定义，因此无需单独安装其他 FFI 包。
...
***TaiLerDownloader*** 已提供类型定义，***skipLibCheck: true*** 可避免潜在的库类型冲突。

---

## 5. 处理动态库路径

***TaiLerDownloader*** 内部已经实现了智能的路径搜索（参见 ***native.ts*** 中的 ***resolveDllPath*** 函数），它会按以下顺序自动查找动态库：
...
```typescript
import { TaiLerDownloader } from 'TaiLerDownloader';
import path from 'path';

const libPath = path.join(__dirname, '../resources/TaiLerDownloader.dll'); // 根据平台调整
const downloader = new TaiLerDownloader({ dllPath: libPath });
```

### 5.1 开发环境建议

为了方便开发，建议将动态库文件放在项目根目录的 ***resources*** 文件夹中，然后在代码中使用自动搜索（不传 ***dllPath***），因为 ***TaiLerDownloader*** 默认会在 ***process.cwd()*** 中查找，而 ***process.cwd()*** 通常就是项目根目录。

### 5.2 打包后路径处理

***TaiLerDownloader*** 会自动识别 Electron 的 ***app.asar.unpacked*** 目录。您只需在打包配置中将动态库文件复制到该目录即可（详见 [第 10 节](#10-打包应用)）。

---

## 6. 在主进程中使用 TaiLerDownloader
...
let downloader: TaiLerDownloader;

app.whenReady().then(() => {
  // 初始化下载器（不传路径则自动搜索）
  downloader = new TaiLerDownloader();
...
由于 Electron 使用自己的 Node 运行时，必须重新编译 ***TaiLerDownloader*** 所依赖的 ***koffi*** 原生模块。在 ***package.json*** 中添加脚本：
...
将动态库文件（***TaiLerDownloader.dll*** / ***TaiLerDownloader.dylib*** / ***TaiLerDownloader.so***）放入项目根目录的 ***resources*** 文件夹。
...
打包后，***TaiLerDownloader*** 会自动在 ***resources/app.asar.unpacked*** 目录中找到动态库，无需额外代码。

---

## 11. TLD 的优势

相比 TTHSD Golang 版本，在 Electron 中使用 TLD 可以获得以下优势：
...
- **直接替换**：只需替换动态库文件即可迁移
- **代码无需修改**：现有代码可以直接使用（注意包名改为 ***TaiLerDownloader***）

---

## 12. 常见问题

### Q1: 编译时提示找不到模块 ***TaiLerDownloader*** 的类型声明

**解决方法**：***TaiLerDownloader*** 已内置类型定义，通常无需额外操作。确保您的 ***tsconfig.json*** 中 ***moduleResolution*** 设置为 ***node***，并且 ***node_modules*** 被正确解析。

### Q2: 运行时错误 "Dynamic Linking Error" 或 "无法加载动态库"

- 检查动态库路径是否正确：***TaiLerDownloader*** 自动搜索路径包括 ***resources/app.asar.unpacked***、***process.cwd()*** 等。可以尝试在构造函数中显式传入绝对路径调试。
- 确认动态库架构与 Electron 匹配（例如均为 64 位）。
- 在 Windows 上，确保系统已安装必要的 VC++ 运行库。

### Q3: 回调函数未触发

- 确认 ***callback*** 参数已传入，且回调函数没有被意外释放。***TaiLerDownloader*** 内部已使用 Map 保存回调引用，只要下载器实例存活，回调就会一直有效。
- 检查动态库是否正确调用回调，可以通过 ***EventLogger*** 内置类快速测试：
  ```typescript
  import { EventLogger } from 'TaiLerDownloader';
...
- 在代码中打印 ***process.resourcesPath*** 和最终路径，确认文件存在。***TaiLerDownloader*** 会自动搜索 ***resources/app.asar.unpacked***，无需手动拼接路径。
...
***TaiLerDownloader*** 还提供了 ***quickDownload*** 函数，返回 ***Promise&lt;void&gt;***，适合简单场景。但在 Electron 中，由于它内部会创建新的 ***TaiLerDownloader*** 实例，您可能需要自行管理生命周期。建议仅在主进程的简单任务中使用。
...
my-electron-app/
├── resources/
│   ├── TaiLerDownloader.dll   (Windows)
│   ├── TaiLerDownloader.dylib (macOS)
│   └── TaiLerDownloader.so    (Linux)
├── src/
│   ├── main.ts                      (主进程)
│   ├── preload.ts                   (预加载脚本)
│   └── renderer/
...
如有问题，请参考 [TLD GitHub](https://github.com/TaiLerDownloader/TaiLerDownloader) 、 [TaiLerDownloader-interface (TypeScript) GitHub](https://github.com/sxxyrry/TaiLerDownloader-interface-ts) 或 [API 文档](/zh/api/API-overview.md)。祝集成顺利！
